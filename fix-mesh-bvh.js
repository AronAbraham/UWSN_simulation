...
// The fixed content for ExtensionUtilities.js
const fixedContent = `import { Ray, Matrix4, Mesh, Vector3, Sphere, REVISION } from 'three';
import { convertRaycastIntersect } from './GeometryRayIntersectUtilities.js';
import { MeshBVH } from '../core/MeshBVH.js';

// Create a mock BatchedMesh class since it's not available in three.js v0.149.0
class BatchedMesh {
  constructor() {
...
// Add necessary prototype functions
BatchedMesh.prototype = {
  raycast: function() {}
};
...
const origBatchedRaycastFunc = BatchedMesh.prototype.raycast; // Using our mock
...
const targetFilePath = path.join(__dirname, 'node_modules', 'three-mesh-bvh', 'src', 'utils', 'ExtensionUtilities.js');
...
try {
  ...
  // Write fixed content to the file
  fs.writeFileSync(targetFilePath, fixedContent);
  console.log('✅ Successfully patched three-mesh-bvh module!');
} catch (error) {
  console.error('❌ Error patching three-mesh-bvh module:', error);
  process.exit(1);
} 